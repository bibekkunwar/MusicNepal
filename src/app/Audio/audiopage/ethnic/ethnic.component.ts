    ...
    this.audioService.getAudioData().subscribe(
      (data: any) => {
        if (data.status && data.data && Array.isArray(data.data)) {
          const ethnicSection = data.data.find((section: any) => section.title === 'Ethnic');
          if (ethnicSection && ethnicSection.albums && Array.isArray(ethnicSection.albums)) {

            this.ethnicItems = ethnicSection.albums;
          } else {
