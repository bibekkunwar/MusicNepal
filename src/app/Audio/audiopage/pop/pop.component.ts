    ...
    this.audioService.getAudioData().subscribe(
      (data: any) => {
        ...
        if (data.status && data.data && Array.isArray(data.data)) {
          const popSection = data.data.find((section: any) => section.title === 'Pop');
          if (popSection && popSection.albums && Array.isArray(popSection.albums)) {

            this.popItems = popSection.albums;
          } else {
